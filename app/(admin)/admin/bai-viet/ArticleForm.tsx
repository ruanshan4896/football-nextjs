...
import RichTextEditor from '@/components/ui/RichTextEditorWrapper'
...
export default function ArticleForm({ initialData }: Props) {
  ...
  async function handleSubmit(status: 'draft' | 'published') {
    ...
    if (form.content_type === 'page_content' && !form.page_path.trim()) {
      setError('Vui lòng nhập Page Path cho nội dung trang.')
      return
    }
    ...
    const payload = {
      ...
      page_type: form.content_type === 'page_content' ? 'general' : null,
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg bg-blue-50 border border-blue-200 p-3">
        ...
        <ul className="text-xs text-blue-700 space-y-1 list-disc list-inside">
          <li><strong>Bài viết:</strong> Nhận định trận đấu, tin tức - hiển thị ở /nhan-dinh</li>
          <li><strong>Nội dung trang:</strong> Giới thiệu, hướng dẫn - hiển thị ở path cụ thể</li>
        </ul>
      </div>
        ...
        {form.content_type === 'page_content' && (
          <div>
            ...
            <input
              ...
              placeholder="/giai-dau/39 hoặc /ty-le-keo"
            ...
            <p className="mt-1 text-xs text-gray-400">
              Nhập đường dẫn đầy đủ (có thể có query params)
            </p>
          </div>
        )}
      </div>
      ...
      {form.content_type === 'page_content' && (
        <div className="rounded-lg bg-yellow-50 border border-yellow-200 p-3">
          <p className="text-xs text-yellow-800 font-medium mb-1">📍 Ví dụ Page Paths:</p>
          <ul className="text-xs text-yellow-700 space-y-0.5 list-disc list-inside">
            <li><code>/giai-dau/39</code> - Giới thiệu Premier League</li>
            <li><code>/doi-bong/33</code> - Giới thiệu Manchester United</li>
            <li><code>/ty-le-keo</code> - Hướng dẫn tỷ lệ kèo</li>
            <li><code>/bang-xep-hang</code> - Hướng dẫn bảng xếp hạng</li>
            <li><code>/lich-thi-dau</code> - Hướng dẫn lịch thi đấu</li>
            <li><code>/bang-xep-hang?league=135</code> - Với query params</li>
          </ul>
        </div>
      )}
      ...
      {/* 2 cột: Match ID + League ID (chỉ để backward compatibility) */}
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">
            Match ID
